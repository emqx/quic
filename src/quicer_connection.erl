...
-type cb_init_args() ::
    #{
        stream_opts => stream_opts() | user_opts(),
        conn => connection_handle()
    }
    | conn_opts().
...
-callback peer_needs_streams(connection_handle(), unidi_streams | bidi_streams, cb_state()) ->
    cb_ret().
...
-callback dgram_recv(
    connection_handle(), DataBin :: binary(), Flags :: non_neg_integer(), cb_state()
) -> cb_ret().
...
start_link(CallbackModule, {_Host, _Port} = Peer, {_COpts, _SOpts} = Opts) when
...
-spec start_link(
    CallbackModule :: undefined | module(),
...
%% For Client
init([CallbackModule, {Host, Port}, {COpts, SOpts}]) when
...
handle_info(
    ...
    ?tp_ignore_side_effects_in_prod(debug, #{
        module => ?MODULE, conn => C, props => Props, event => connected
    }),
...
handle_info(
    ...
    ?tp_ignore_side_effects_in_prod(debug, #{
        module => ?MODULE, conn => C, event => transport_shutdown
    }),
    default_cb_ret(M:transport_shutdown(C, DownInfo, CbState), State);
handle_info(
    {quic, shutdown, C, ErrorCode},
    #{
        conn := C,
        callback := M,
        callback_state := CbState
...
handle_info(
    {quic, closed, C, #{is_app_closing := false} = Flags},
    #{
        conn := C,
        callback := M,
        callback_state := CBState
...
handle_info(
    {quic, local_address_changed, C, NewAddr},
    #{
        conn := C,
        callback := M,
        callback_state := CBState
    ...
    ?tp_ignore_side_effects_in_prod(debug, #{
        module => ?MODULE, conn => C, event => local_address_changed, new_addr => NewAddr
    }),
    default_cb_ret(M:local_address_changed(C, NewAddr, CBState), State);
handle_info(
    {quic, peer_address_changed, C, NewAddr},
    #{
        conn := C,
        callback := M,
        callback_state := CbState
    ...
    ?tp_ignore_side_effects_in_prod(debug, #{
        module => ?MODULE, conn => C, event => peer_address_changed, new_addr => NewAddr
    }),
    default_cb_ret(M:peer_address_changed(C, NewAddr, CbState), State);
handle_info(
    {quic, new_stream, Stream, Props},
    #{
        conn := C,
        callback := M,
        callback_state := CbState
