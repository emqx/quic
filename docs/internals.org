...
| QUIC_CONNECTION_EVENT_SHUTDOWN_INITIATED_BY_PEER      | {quic, shutdown, connection_handler(),atom_status()} @TODO errcode         |
| QUIC_CONNECTION_EVENT_SHUTDOWN_COMPLETE               | {quic, closed, connection_handler()} @TODO flags                           |
...
| QUIC_CONNECTION_EVENT_PEER_STREAM_STARTED             | {quic, new_stream, stream_handler()} @TODO stream                          |
...
| QUIC_CONNECTION_EVENT_RESUMED                         | @TODO                                                                      |
...
| MsQuic Event                             | Erlang Msg                                                                                   |
|------------------------------------------+----------------------------------------------------------------------------------------------|
| QUIC_STREAM_EVENT_START_COMPLETE         | {quic, start_completed, stream_handler(), atom_status(), stream_id(), peer_accepted()}       |
| QUIC_STREAM_EVENT_RECEIVE                | {quic, stream_handler(), continue} or {quic, binary(), stream_handler(), Offset, Len, Flags} |
| QUIC_STREAM_EVENT_SEND_COMPLETE          | {quic, send_complete, stream_handler(), CanceledFlag}                                        |
| QUIC_STREAM_EVENT_PEER_SEND_SHUTDOWN     | {quic, peer_send_shutdown, stream_handler()}                                                 |
...
| QUIC_STREAM_EVENT_SHUTDOWN_COMPLETE      | {quic, closed, stream_handler(), IsConnCloseFlag()}                                          |
| QUIC_STREAM_EVENT_IDEAL_SEND_BUFFER_SIZE | @TODO                                                                                        |
| QUIC_STREAM_EVENT_PEER_ACCEPTED          | @TODO                                                                                        |
...
- quicer_server_stream_callback
  ...
  Example callback module for handle *server* streams.
