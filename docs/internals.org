...
The NIF (`quicer_nif:recv/2`) first check if `is_recv_pending` is set and there are enough bytes required.
...
NOTE, duplicate `StreamReceiveComplete` calls could cause undefined behaviors.

If FALSE, NIF will mark `is_wait_for_data` in stream ctx and return `{error, not_ready}` to the caller and the caller should
wait for recv `continue` message mentioned above. NIF also calls `StreamReceiveComplete` with 0 consumed data iff `is_recv_pending`
is SET in order to complete the *async recv* and then reenabling the recv callback by calling `StreamReceiveSetEnabled` because of
zero consumed data. see [MsQuic/docs/Streams.md]

*** quicer:recv/2 call in Active Mode

Should return error.

*** Mode switching from passive to active
